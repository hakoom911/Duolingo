...
export async function GET(req: NextRequest) {
    ...
    const completedChallenges = normalizedChallenges?.filter((challenge)=> challenge.completed)
    const percentage = completedChallenges && normalizedChallenges ? Math.round((completedChallenges.length / normalizedChallenges.length) * 100, ): 0
    const result = {
